...
const userStatsLeaderboardCommand = {
  async execute(interaction: any) {
    ...
    leaderboard.forEach((row, idx) => {
      ...
      message += `${idx + 1}. ${statusEmoji} **${username}** | 🎲 **${row.totalPredictions ?? 0}** = ✅ ​${row.correctPredictions ?? 0} + ⏹️ ​${row.noWinnersPredictions ?? 0} + ❌ ${row.incorrectPredictions ?? 0} | ⭐ ${winRate} | 💠 ${row.auraPoints ?? 0} | 🔥 ${row.streak ?? 0} | 🪙​ **${totalFormateado}**\n`;
    });
